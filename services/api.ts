...
const fetchWithCache = async (cacheKey: string, url: string, options: RequestInit = {}, ttlSeconds: number) => {
...
export const fetchDefiLlamaTVL = async () => {
  try {
    // Cache for 1 hour
    const data = await fetchWithCache('defillama_tvl', 'https://api.llama.fi/v2/chains', {}, 3600);
    return data;
  } catch (error) {
...
export const fetchMacroIndicators = async () => {
  try {
    ...
    const symbols = 'DX-Y.NYB,^GSPC,^IXIC,GCUSD,SIUSD,^TNX,SPY,QQQ,AAPL,TSLA,NVDA';
    const url = `${FMP_BASE_URL}/quote/${symbols}?apikey=${FMP_API_KEY}`;
    // Cache macro indices for 60 seconds
    const data = await fetchWithCache('fmp_macro_quotes', url, {}, 60);
    return data;
  } catch (error) {
...
export const fetchForexRates = async (baseCurrency = 'USD') => {
  try {
    ...
    // Cache forex rates for 1 hour
    const data = await fetchWithCache(`forex_latest_${baseCurrency}`, url, {}, 3600);
    return data;
  } catch (error) {
